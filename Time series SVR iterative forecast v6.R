# Time series SVR iterative forecast
# 
...
# rmse_svr
rmse_svr <- function(symbol,setplot=0) {
	...
	current_seq <- data[(length(data) - lookback):length(data)]		# last sequence of actual data - lookback
	...
	for (i in seq(future_steps)) {
	    ...
	    next_value <- model_svrt %>% predict(tc)				# predict next value for tc  
	...
	if (setplot==1){
		png(filename=paste0("c:\\ml\\SVR_", symbol, ".png"))
	...
	r <- rmse(data_raw[(length(data_raw)-4*7+1):(length(data_raw))],
		future_predictions * (max(data_raw) - min(data_raw)) + min(data_raw))

	write.csv(data.frame(rmse=c(r)),paste0("c:\\ml\\SVR_", symbol, ".csv"),row.names = FALSE)

	return (r)
}

print(paste0("SVR Tesla:",	rmse_svr("TSLA",1)))
print(paste0("SVR Google:", 	rmse_svr("GOOGL",1)))
print(paste0("SVR Nvidia:", 	rmse_svr("NVDA",1)))
print(paste0("SVR Apple:", 	rmse_svr("AAPL",1)))
print(paste0("SVR Microsoft:",	rmse_svr("MSFT",1)))
print(paste0("SVR Nokia:", 	rmse_svr("NOK",1)))
print(paste0("SVR Rheinmetall:", rmse_svr("RHM.F",1)))
print(paste0("SVR BAE Systems:", rmse_svr("BA.L",1)))
print(paste0("SVR Dassault Av.", rmse_svr("AM.PA",1)))
print(paste0("SVR Thalès:",	rmse_svr("HO.PA",1)))
print(paste0("SVR Leonardo:",	rmse_svr("FMNB.F",1)))
